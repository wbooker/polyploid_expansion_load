...
for(stat in 1:length(stats)){
  file <- stats[stat]
mod_dir <- list.dirs(path = "/work/users/w/w/wwbooker/polyploid_expansion_load/output/revision_run/basic", full.names = TRUE, recursive = FALSE)

for(mod in mod_dir[c(50:52)]){
  file <- c("_prevFst")
...
#    for(i in c(1:11,13:18,20,23:25)){
  for(i in c(1:100)){
    ...
    #image.plot(x,y,as.matrix(z), zlim=c(0.85,1.05), xlab="Generation", ylab="Deme", las = 1)
    image.plot(x,y,as.matrix(z), zlim=c(0.85, 1.05), xlab="Generation", ylab="Deme", las = 1)
...
mod_dir <- list.dirs(path = "/work/users/w/w/wwbooker/polyploid_expansion_load/output/revision_run/dip", full.names = TRUE, recursive = FALSE)

  file <- c("_prevFst")
  #mod <- "/work/users/w/w/wwbooker/polyploid_expansion_load/output/test_dip/diploidization-diff_dipLambda-100_remDipMuts-1_meioticfFitness-1_recessive_K-100_m-0.05_r-0.693147_u_del-2.5e-08_u_ben-2.5e-09_u_dip-0.001_rho-1.0e-06_bs-0.0_ds--0.005_g-999999_start-5001"
  for(mod in mod_dir){
  setwd(mod)
  mod_reps <- list.dirs(path = mod, full.names = TRUE, recursive = FALSE)
  init_count = 1

#    for(i in c(1:11,13:18,20,23:25)){
  for(i in c(1:(length(mod_reps)))){
      print(c(file,i))
    ...
    #image.plot(x,y,as.matrix(z), zlim=c(0, 0.01), xlab="Generation", ylab="Deme")
    #image.plot(x,y,as.matrix(z), zlim=c(0,75), xlab="Generation", ylab="Deme")
    image.plot(x,y,as.matrix(z), zlim=c(0,0.3), xlab="Generation", ylab="Deme", las = 1)
    ...
    #image.plot(x,y,as.matrix(z), zlim=c(0,75), xlab="Generation", ylab="Deme")
    image.plot(x,y,as.matrix(z), zlim=c(0,0.3), xlab="Generation", ylab="Deme", las = 1)
    #image.plot(x,y,as.matrix(z), zlim=c(0,1), xlab="Generation", ylab="Deme", las = 1)    #image.plot(x,y,as.matrix(z), zlim=c(0, 0.05), xlab="Generation", ylab="Deme")
    #image.plot(x,y,as.matrix(z), zlim=c(0.000000001, 0.0001), xlab="Generation", ylab="Deme")
    #image.plot(x,y,as.matrix(z), zlim=c(-5, 5), xlab="Generation", ylab="Deme")
    #image.plot(x,y,as.matrix(z), zlim=c(0, 1), xlab="Generation", ylab="Deme", col = plot_colors)
